...
public class TreasureCardController implements Initializable {
	...
	private static final String MONSTER_DEFEATED_TEXT = "You have already defeated a monster   +2 Gold";
	private static final String MONSTER_NOT_DEFEATED_TEXT = "You have NOT yet defeated a monster   +1 Gold";
	private static final String ROLL_POSSIBLE_TEXT = "You CAN roll again for item!";
	private static final String ROLL_NOT_POSSIBLE_TEXT = "You CANNOT roll again for item!";
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		if (character.isMonsterDefeated()) {
			monsterDefeatedLabel.setText(MONSTER_DEFEATED_TEXT);
			character.setGold(character.getGold() + 2);
		} else {
			monsterDefeatedLabel.setText(MONSTER_NOT_DEFEATED_TEXT);
	...
	public void checkIfRollForItemPossible() {
		int diceResult = diceRoller.getDice().get(0).getValue();
		if (diceResult >= 5) {
			resultLabel.setText(ROLL_POSSIBLE_TEXT);
			rollingForItem = true;
		} else {
			resultLabel.setText(ROLL_NOT_POSSIBLE_TEXT);
			disableRollButton();
			gameEngine.setRoundFinished(true);
		}
	}
